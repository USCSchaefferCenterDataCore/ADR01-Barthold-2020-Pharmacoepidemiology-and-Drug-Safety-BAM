...
putexcel set "/disk/agedisk3/medicare.work/goldman-DUA51866/barthold-dua51866/bam/logit_bam_adrd_8f.xlsx", sheet(Sum) replace
...
//nsl vs m3s
	//at each level
		foreach le in `levelsc'{
			...
			drop if nsl_ctddc_`le'==1 & m3s_ctddc_`le'==1 
			logit $yvar nsl_ctddc_`le' $controls $bldx, or vce(cluster county)
			matrix logit1=r(table)'
			putexcel set "/disk/agedisk3/medicare.work/goldman-DUA51866/barthold-dua51866/bam/logit_bam_adrd_8f.xlsx", sheet(allc `le') modify
