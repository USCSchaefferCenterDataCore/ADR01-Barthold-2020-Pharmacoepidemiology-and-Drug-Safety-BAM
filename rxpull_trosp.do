...
gen trosp = 1 if strpos(gname, "TROSPIUM")
...
///////////////// Array
forvalues d = 1/365{
	gen trosp_a`d' = 1 if `d'>=doy_srvc & `d'<=(doy_srvc+dayssply2) & trosp==1
}
...
/////////////////bene-class timing
egen trosp_fdays_2006 = sum(dayssply), by(idn)
egen trosp_clms_2006 = count(dayssply), by(idn)
egen trosp_minfilldt_2006 = min(srvc_dt), by(idn)
egen trosp_maxfilldt_2006 = max(srvc_dt), by(idn)
gen trosp_fillperiod_2006 = trosp_maxfilldt_2006 - trosp_minfilldt_2006 + 1

//////////////// weighted average dose
//claim_mgs is calculated dependent on dose form (at the claim level). 
gen claim_mgs = 0
replace claim_mgs = 20*qtydspns if str=="20MG"
replace claim_mgs = 20*qtydspns if str=="20 MG"
replace claim_mgs = 60*qtydspns if str=="60MG"
replace claim_mgs = 60*qtydspns if str=="60 MG"
//sum mgs for all claims in year by bene
egen itot_mgs = sum(claim_mgs), by(idn)
//average mgs per day supplied
gen trosp_dmg_2006 = itot_mgs/trosp_fdays_2006
...
forvalues d = 1/365{
	replace snf_push = snf_push + 1 if trosp_a`d'==1 & snf_a`d'==1

	//if the trosp_a`d' is 1, do nothing (already added it to the snf_push)
	//if that trosp_a spot is empty:
	gen v1 = 1 if snf_push>0 & trosp_a`d'==.
	replace trosp_a`d' = 1 if v1==1
...
forvalues d = 1/365{
	replace ips_push = ips_push + 1 if trosp_a`d'==1 & ips_a`d'==1

	//if the trosp_a`d' is 1, do nothing (already added it to the ips_push)
	//if that trosp_a spot is not full:
	gen v1 = 1 if ips_push>0 & trosp_a`d'==.
	replace trosp_a`d' = 1 if v1==1
...
*/
forvalues d = 1/365{
	replace trosp_pdays_2006 = trosp_pdays_2006 + 1 if trosp_a`d'==1
}
drop trosp_a*
replace trosp_pdays_2006 = 365 if trosp_pdays_2006>365

//	mgs in year, for all the possession days
gen trosp_totmg_2006 = trosp_pdays_2006*trosp_dmg_2006
//	total daily doses in the year
gen trosp_totdd_2006 = trosp_totmg_2006/20
...
egen trosp_push_2006 = rowmax(lastfill_push inyear_push) 
replace trosp_push_2006 = 0 if trosp_push_2006==.
replace trosp_push_2006 = 0 if trosp_push_2006<0
...
replace pushstock = pushstock + trosp_push_2006 if bene_id!=bene_id[_n-1] | _n==1
...
/////////////////bene-class timing
egen trosp_fdays_2007 = sum(dayssply), by(idn)
egen trosp_clms_2007 = count(dayssply), by(idn)
egen trosp_minfilldt_2007 = min(srvc_dt), by(idn)
egen trosp_maxfilldt_2007 = max(srvc_dt), by(idn)
gen trosp_fillperiod_2007 = trosp_maxfilldt_2007 - trosp_minfilldt_2007 + 1
...
egen itot_mgs = sum(claim_mgs), by(idn)
//average mgs per day supplied
gen trosp_dmg_2007 = itot_mgs/trosp_fdays_2007
drop claim_mgs itot_mgs
...
*/
forvalues d = 1/365{
	replace trosp_pdays_2007 = trosp_pdays_2007 + 1 if trosp_a`d'==1
}
drop trosp_a*
replace trosp_pdays_2007 = 365 if trosp_pdays_2007>365

//	mgs in year, for all the possession days
gen trosp_totmg_2007 = trosp_pdays_2007*trosp_dmg_2007
//	total daily doses in the year
gen trosp_totdd_2007 = trosp_totmg_2007/20
...
gen inyear_push = earlyfill_push + snf_push + ips_push
replace inyear_push = 30 if inyear_push>30 & inyear_push!=.
egen trosp_push_2007 = rowmax(lastfill_push inyear_push) 
replace trosp_push_2007 = 0 if trosp_push_2007==.
replace trosp_push_2007 = 0 if trosp_push_2007<0
...
//add the pushstock from last year
replace pushstock = pushstock + trosp_push_2007 if bene_id!=bene_id[_n-1] | _n==1
...
/////////////////bene-class timing
egen trosp_fdays_2008 = sum(dayssply), by(idn)
egen trosp_clms_2008 = count(dayssply), by(idn)
egen trosp_minfilldt_2008 = min(srvc_dt), by(idn)
egen trosp_maxfilldt_2008 = max(srvc_dt), by(idn)
gen trosp_fillperiod_2008 = trosp_maxfilldt_2008 - trosp_minfilldt_2008 + 1
...
egen itot_mgs = sum(claim_mgs), by(idn)
//average mgs per day supplied
gen trosp_dmg_2008 = itot_mgs/trosp_fdays_2008
drop claim_mgs itot_mgs
...
*/
forvalues d = 1/365{
	replace trosp_pdays_2008 = trosp_pdays_2008 + 1 if trosp_a`d'==1
}
drop trosp_a*
replace trosp_pdays_2008 = 365 if trosp_pdays_2008>365

//	mgs in year, for all the possession days
gen trosp_totmg_2008 = trosp_pdays_2008*trosp_dmg_2008
//	total daily doses in the year
gen trosp_totdd_2008 = trosp_totmg_2008/20
...
gen inyear_push = earlyfill_push + snf_push + ips_push
replace inyear_push = 30 if inyear_push>30 & inyear_push!=.
egen trosp_push_2008 = rowmax(lastfill_push inyear_push) 
replace trosp_push_2008 = 0 if trosp_push_2008==.
replace trosp_push_2008 = 0 if trosp_push_2008<0
...
//add the pushstock from last year
replace pushstock = pushstock + trosp_push_2008 if bene_id!=bene_id[_n-1] | _n==1
...
/////////////////bene-class timing
egen trosp_fdays_2009 = sum(dayssply), by(idn)
egen trosp_clms_2009 = count(dayssply), by(idn)
egen trosp_minfilldt_2009 = min(srvc_dt), by(idn)
egen trosp_maxfilldt_2009 = max(srvc_dt), by(idn)
gen trosp_fillperiod_2009 = trosp_maxfilldt_2009 - trosp_minfilldt_2009 + 1
...
egen itot_mgs = sum(claim_mgs), by(idn)
//average mgs per day supplied
gen trosp_dmg_2009 = itot_mgs/trosp_fdays_2009
drop claim_mgs itot_mgs
...
*/
forvalues d = 1/365{
	replace trosp_pdays_2009 = trosp_pdays_2009 + 1 if trosp_a`d'==1
}
drop trosp_a*
replace trosp_pdays_2009 = 365 if trosp_pdays_2009>365

//	mgs in year, for all the possession days
gen trosp_totmg_2009 = trosp_pdays_2009*trosp_dmg_2009
//	total daily doses in the year
gen trosp_totdd_2009 = trosp_totmg_2009/20
...
gen inyear_push = earlyfill_push + snf_push + ips_push
replace inyear_push = 30 if inyear_push>30 & inyear_push!=.
egen trosp_push_2009 = rowmax(lastfill_push inyear_push) 
replace trosp_push_2009 = 0 if trosp_push_2009==.
replace trosp_push_2009 = 0 if trosp_push_2009<0
...
//add the pushstock from last year
replace pushstock = pushstock + trosp_push_2009 if bene_id!=bene_id[_n-1] | _n==1
...
/////////////////bene-class timing
egen trosp_fdays_2010 = sum(dayssply), by(idn)
egen trosp_clms_2010 = count(dayssply), by(idn)
egen trosp_minfilldt_2010 = min(srvc_dt), by(idn)
egen trosp_maxfilldt_2010 = max(srvc_dt), by(idn)
gen trosp_fillperiod_2010 = trosp_maxfilldt_2010 - trosp_minfilldt_2010 + 1
...
egen itot_mgs = sum(claim_mgs), by(idn)
//average mgs per day supplied
gen trosp_dmg_2010 = itot_mgs/trosp_fdays_2010
drop claim_mgs itot_mgs
...
*/
forvalues d = 1/365{
	replace trosp_pdays_2010 = trosp_pdays_2010 + 1 if trosp_a`d'==1
}
drop trosp_a*
replace trosp_pdays_2010 = 365 if trosp_pdays_2010>365

//	mgs in year, for all the possession days
gen trosp_totmg_2010 = trosp_pdays_2010*trosp_dmg_2010
//	total daily doses in the year
gen trosp_totdd_2010 = trosp_totmg_2010/20
...
gen inyear_push = earlyfill_push + snf_push + ips_push
replace inyear_push = 30 if inyear_push>30 & inyear_push!=.
egen trosp_push_2010 = rowmax(lastfill_push inyear_push) 
replace trosp_push_2010 = 0 if trosp_push_2010==.
replace trosp_push_2010 = 0 if trosp_push_2010<0
...
//add the pushstock from last year
replace pushstock = pushstock + trosp_push_2010 if bene_id!=bene_id[_n-1] | _n==1
...
/////////////////bene-class timing
egen trosp_fdays_2011 = sum(dayssply), by(idn)
egen trosp_clms_2011 = count(dayssply), by(idn)
egen trosp_minfilldt_2011 = min(srvc_dt), by(idn)
egen trosp_maxfilldt_2011 = max(srvc_dt), by(idn)
gen trosp_fillperiod_2011 = trosp_maxfilldt_2011 - trosp_minfilldt_2011 + 1
...
egen itot_mgs = sum(claim_mgs), by(idn)
//average mgs per day supplied
gen trosp_dmg_2011 = itot_mgs/trosp_fdays_2011
drop claim_mgs itot_mgs
...
*/
forvalues d = 1/365{
	replace trosp_pdays_2011 = trosp_pdays_2011 + 1 if trosp_a`d'==1
}
drop trosp_a*
replace trosp_pdays_2011 = 365 if trosp_pdays_2011>365

//	mgs in year, for all the possession days
gen trosp_totmg_2011 = trosp_pdays_2011*trosp_dmg_2011
//	total daily doses in the year
gen trosp_totdd_2011 = trosp_totmg_2011/20
...
gen inyear_push = earlyfill_push + snf_push + ips_push
replace inyear_push = 30 if inyear_push>30 & inyear_push!=.
egen trosp_push_2011 = rowmax(lastfill_push inyear_push) 
replace trosp_push_2011 = 0 if trosp_push_2011==.
replace trosp_push_2011 = 0 if trosp_push_2011<0
...
//add the pushstock from last year
replace pushstock = pushstock + trosp_push_2011 if bene_id!=bene_id[_n-1] | _n==1
...
/////////////////bene-class timing
egen trosp_fdays_2012 = sum(dayssply), by(idn)
egen trosp_clms_2012 = count(dayssply), by(idn)
egen trosp_minfilldt_2012 = min(srvc_dt), by(idn)
egen trosp_maxfilldt_2012 = max(srvc_dt), by(idn)
gen trosp_fillperiod_2012 = trosp_maxfilldt_2012 - trosp_minfilldt_2012 + 1
...
egen itot_mgs = sum(claim_mgs), by(idn)
//average mgs per day supplied
gen trosp_dmg_2012 = itot_mgs/trosp_fdays_2012
drop claim_mgs itot_mgs
...
*/
forvalues d = 1/365{
	replace trosp_pdays_2012 = trosp_pdays_2012 + 1 if trosp_a`d'==1
}
drop trosp_a*
replace trosp_pdays_2012 = 365 if trosp_pdays_2012>365

//	mgs in year, for all the possession days
gen trosp_totmg_2012 = trosp_pdays_2012*trosp_dmg_2012
//	total daily doses in the year
gen trosp_totdd_2012 = trosp_totmg_2012/20
...
gen inyear_push = earlyfill_push + snf_push + ips_push
replace inyear_push = 30 if inyear_push>30 & inyear_push!=.
egen trosp_push_2012 = rowmax(lastfill_push inyear_push) 
replace trosp_push_2012 = 0 if trosp_push_2012==.
replace trosp_push_2012 = 0 if trosp_push_2012<0
...
//add the pushstock from last year
replace pushstock = pushstock + trosp_push_2012 if bene_id!=bene_id[_n-1] | _n==1
...
/////////////////bene-class timing
egen trosp_fdays_2013 = sum(dayssply), by(idn)
egen trosp_clms_2013 = count(dayssply), by(idn)
egen trosp_minfilldt_2013 = min(srvc_dt), by(idn)
egen trosp_maxfilldt_2013 = max(srvc_dt), by(idn)
gen trosp_fillperiod_2013 = trosp_maxfilldt_2013 - trosp_minfilldt_2013 + 1
...
egen itot_mgs = sum(claim_mgs), by(idn)
//average mgs per day supplied
gen trosp_dmg_2013 = itot_mgs/trosp_fdays_2013
drop claim_mgs itot_mgs
...
*/
forvalues d = 1/365{
	replace trosp_pdays_2013 = trosp_pdays_2013 + 1 if trosp_a`d'==1
}
drop trosp_a*
replace trosp_pdays_2013 = 365 if trosp_pdays_2013>365

//	mgs in year, for all the possession days
gen trosp_totmg_2013 = trosp_pdays_2013*trosp_dmg_2013
//	total daily doses in the year
gen trosp_totdd_2013 = trosp_totmg_2013/20
...
gen inyear_push = earlyfill_push + snf_push + ips_push
replace inyear_push = 30 if inyear_push>30 & inyear_push!=.
egen trosp_push_2013 = rowmax(lastfill_push inyear_push) 
replace trosp_push_2013 = 0 if trosp_push_2013==.
replace trosp_push_2013 = 0 if trosp_push_2013<0
...
//add the pushstock from last year
replace pushstock = pushstock + trosp_push_2013 if bene_id!=bene_id[_n-1] | _n==1
...
/////////////////bene-class timing
egen trosp_fdays_2014 = sum(dayssply), by(idn)
egen trosp_clms_2014 = count(dayssply), by(idn)
egen trosp_minfilldt_2014 = min(srvc_dt), by(idn)
egen trosp_maxfilldt_2014 = max(srvc_dt), by(idn)
gen trosp_fillperiod_2014 = trosp_maxfilldt_2014 - trosp_minfilldt_2014 + 1
...
egen itot_mgs = sum(claim_mgs), by(idn)
//average mgs per day supplied
gen trosp_dmg_2014 = itot_mgs/trosp_fdays_2014
drop claim_mgs itot_mgs
...
*/
forvalues d = 1/365{
	replace trosp_pdays_2014 = trosp_pdays_2014 + 1 if trosp_a`d'==1
}
drop trosp_a*
replace trosp_pdays_2014 = 365 if trosp_pdays_2014>365

//	mgs in year, for all the possession days
gen trosp_totmg_2014 = trosp_pdays_2014*trosp_dmg_2014
//	total daily doses in the year
gen trosp_totdd_2014 = trosp_totmg_2014/20
...
gen inyear_push = earlyfill_push + snf_push + ips_push
replace inyear_push = 30 if inyear_push>30 & inyear_push!=.
egen trosp_push_2014 = rowmax(lastfill_push inyear_push) 
replace trosp_push_2014 = 0 if trosp_push_2014==.
replace trosp_push_2014 = 0 if trosp_push_2014<0
