...
putexcel set "/disk/agedisk3/medicare.work/goldman-DUA51866/barthold-dua51866/bam/logit_bam_adrd_8b.xlsx", sheet(Sum) replace
...
//nsl vs m3s
	//at each level
		foreach le in `levelsc'{
			restore 
			preserve
			keep if nsl2_ctddc_`le'==1 | m3s_ctddc_`le'==1 
			logit $yvar nsl2_ctddc_`le' $controls $bldx, or vce(cluster county)
			matrix logit1=r(table)'
			putexcel set "/disk/agedisk3/medicare.work/goldman-DUA51866/barthold-dua51866/bam/logit_bam_adrd_8b.xlsx", sheet(all `le') modify
			putexcel A1="Compare drug	s to other drugs", bold
			...
			sum nsl2_ctddc_`le' if nsl2_ctddc_`le'==1
	...
	//female
		foreach le in `levelsc'{
			...
			logit $yvar nsl2_ctddc_`le' $controls $bldx, or vce(cluster county)
			matrix logit1=r(table)'
			putexcel set "/disk/agedisk3/medicare.work/goldman-DUA51866/barthold-dua51866/bam/logit_bam_adrd_8b.xlsx", sheet(fem `le') modify
	...
	//male
		foreach le in `levelsc'{
			...
			logit $yvar nsl2_ctddc_`le' $controls $bldx, or vce(cluster county)
			matrix logit1=r(table)'
			putexcel set "/disk/agedisk3/medicare.work/goldman-DUA51866/barthold-dua51866/bam/logit_bam_adrd_8b.xlsx", sheet(mal `le') modify
			putexcel A1="Compare drugs to other drugs", bold
			putexcel A2="Logit regressions of ADRD incidence (2011-2016) on exposure to BAM prescription drugs (2007-2009)", bold 
			putexcel B3="Compare nsl to those using m3s at the same level"
			putexcel B4="Sample: tdd `le' of nsl or m3s, no ADRD as of 2011, age 67+, Part D 07-09, FFS 07-09" B5="N =" 
	...
	//white
		foreach le in `levelsc'{
			...
			logit $yvar nsl2_ctddc_`le' $controls $bldx, or vce(cluster county)
			matrix logit1=r(table)'
			putexcel set "/disk/agedisk3/medicare.work/goldman-DUA51866/barthold-dua51866/bam/logit_bam_adrd_8b.xlsx", sheet(whi `le') modify
			putexcel A1="Compare drugs to other drugs", bold
			putexcel A2="Logit regressions of ADRD incidence (2011-2016) on exposure to BAM prescription drugs (2007-2009)", bold 
			putexcel B3="Compare nsl to those using m3s at the same level"
			putexcel B4="Sample: tdd `le' of nsl or m3s, no ADRD as of 2011, age 67+, Part D 07-09, FFS 07-09" B5="N =" 
	...
	//black
		foreach le in `levelsc'{
			...
			logit $yvar nsl2_ctddc_`le' $controls $bldx, or vce(cluster county)
			matrix logit1=r(table)'
			putexcel set "/disk/agedisk3/medicare.work/goldman-DUA51866/barthold-dua51866/bam/logit_bam_adrd_8b.xlsx", sheet(bla `le') modify
			putexcel A1="Compare drugs to other drugs", bold
			putexcel A2="Logit regressions of ADRD incidence (2011-2016) on exposure to BAM prescription drugs (2007-2009)", bold 
			putexcel B3="Compare nsl to those using m3s at the same level"
			putexcel B4="Sample: tdd `le' of nsl or m3s, no ADRD as of 2011, age 67+, Part D 07-09, FFS 07-09" B5="N =" 
	...
	//hisp
		foreach le in `levelsc'{
			...
			logit $yvar nsl2_ctddc_`le' $controls $bldx, or vce(cluster county)
			matrix logit1=r(table)'
			putexcel set "/disk/agedisk3/medicare.work/goldman-DUA51866/barthold-dua51866/bam/logit_bam_adrd_8b.xlsx", sheet(his `le') modify
			putexcel A1="Compare drugs to other drugs", bold
			putexcel A2="Logit regressions of ADRD incidence (2011-2016) on exposure to BAM prescription drugs (2007-2009)", bold 
			putexcel B3="Compare nsl to those using m3s at the same level"
			putexcel B4="Sample: tdd `le' of nsl or m3s, no ADRD as of 2011, age 67+, Part D 07-09, FFS 07-09" B5="N =" 
	...
	//oth
		foreach le in `levelsc'{
			...
			logit $yvar nsl2_ctddc_`le' $controls $bldx, or vce(cluster county)
			matrix logit1=r(table)'
			putexcel set "/disk/agedisk3/medicare.work/goldman-DUA51866/barthold-dua51866/bam/logit_bam_adrd_8b.xlsx", sheet(oth `le') modify
			putexcel A1="Compare drugs to other drugs", bold
			putexcel A2="Logit regressions of ADRD incidence (2011-2016) on exposure to BAM prescription drugs (2007-2009)", bold 
			putexcel B3="Compare nsl to those using m3s at the same level"
			putexcel B4="Sample: tdd `le' of nsl or m3s, no ADRD as of 2011, age 67+, Part D 07-09, FFS 07-09" B5="N =" 
